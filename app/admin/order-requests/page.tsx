...
import { Order, TradeType } from "@/app/types/orders";
    ...
    >
      <div className="flex justify-between items-center mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm">{order.createdAt ? new Date(order.createdAt).toLocaleDateString() : 'N/A'}</span>
        </div>
        <span
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        ...
        <div>
          ...
          <p className="font-medium">₹{(order.quantity * order.buyPrice).toFixed(2)}</p>
        </div>
      </div>
      ...
      <div className="bg-background/80 backdrop-blur-lg rounded-xl border shadow-sm overflow-hidden">
        ...
        <div className="hidden md:block overflow-x-auto">
          {loading ? (
            <div className="p-4">
              <div className="space-y-4">
                <div className="grid grid-cols-11 gap-4 p-3">
                  {[...Array(11)].map((_, i) => (
                ...
                {[...Array(2)].map((_, i) => (
                  <div key={i} className="grid grid-cols-11 gap-4 p-3 border-t">
                    {[...Array(11)].map((_, j) => (
                      <div key={j} className="space-y-2">
                        <div className="h-4 bg-gray-200 rounded w-full animate-pulse"></div>
                        {j === 10 && (
              ...
              <tbody>
                {filteredOrders.map((order, idx) => (
                  ...
                  >
                    ...
                    <td className="p-3 text-right">
                      ₹{(order.quantity * order.buyPrice).toFixed(2)}
                    </td>
                    <td className="p-3">
                      <span
                        className={`px-2 py-1 rounded-full text-xs ${
